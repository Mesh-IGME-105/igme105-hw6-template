...
namespace HW6_CritterFarm
{
    ...
    class CritterManager
    {
        ...
        /// <summary>
        /// Parameterized constructor to initialize the list and active critter and set the filename to use.
        /// </summary>
        ...
        public void SetupCritters()
        {
            ...
            {
                string name = SmartConsole.Prompt("\nEnter critter " + (i + 1) + " name:");
                string typeString = SmartConsole.Prompt("What type of critter is " + name + " (Cat, Dog, or Horse)?");
                ...
                // to proceed.
                {
                    ...
                    typeString = SmartConsole.Prompt("What type of critter is " + name + " (Cat, Dog, or Horse)?");
                }
        ...
        public void LoadCrittersFromFile()
        {
            ...
            // ********************************
            StreamReader reader = null!; // The ! tells the compiler to ignore warnings about a possible null value.
            ...
            try
            {
                ...
                // Read in data
                // If data is missing, get starting data from user
                // If data exists, create Critters and place them in the list
                string? line = "";
                while ((line = reader.ReadLine()) != null)
                {
                    if (line == "No critters saved")
                    {
                        loadManually = true;
                    }
                    else
                    {
                        string[] critterData = line.Split('|');

                        CritterType type = CritterType.Cat;

                        if (Enum.TryParse<CritterType>(critterData[0], true, out type)
                            && Enum.IsDefined(typeof(CritterType), type))
                        {
                            try
                            {
                                // Create the correct type of critter
                                // TODO: If you don't use Cat, Dog, & Horse as your critter types, fix the constructor calls below
                                ...
                                switch (type)
                                {
                                    case CritterType.Cat:
                                        critterList.Add(
                                            new Cat(
                                                critterData[1],
                                                int.Parse(critterData[2]),
                                                int.Parse(critterData[3])));
                                        break;
                        ...
                        {
                            SmartConsole.PrintWarning(critterData[0] + "'s aren't supported yet. Skipping this line: " + line);
                        }
                    }
                }
            }
            ...
            {
                // File does not exist, or contains the wrong information
                // Start from scratch by asking user for critter names
            ...
            // If the load didn't work, setup manaully.
            if (loadManually)
            {
                SmartConsole.PrintError("You'll need to set up a new critter farm.\n");
                SetupCritters();
            }
            else
            {
                SmartConsole.PrintSuccess(this.critterList.Count + " critters loaded successfully.");
            }
        }
